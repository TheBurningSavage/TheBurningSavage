TheBurningSavage Core version 0.16.0 is now available from:

  <https://theburningsavagecore.org/bin/theburningsavage-core-0.16.0/>
...
installer (on Windows) or just copy over `/Applications/TheBurningSavage-Qt` (on Mac)
or `theburningsavaged`/`theburningsavage-qt` (on Linux).
...
TheBurningSavage Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

TheBurningSavage Core should also work on most other Unix-like systems but is not
...
TheBurningSavage Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
theburningsavage data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile TheBurningSavage Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µTBS" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- theburningsavage-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- theburningsavage-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest theburningsavage.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.theburningsavage.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for theburningsavage-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to theburningsavage-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µTBS to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_theburningsavage_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` theburningsavage_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.theburningsavage doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for theburningsavage-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in theburningsavage-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop theburningsavaged (OmeGak)
...
- #10529 `331352f` Improve theburningsavaged systemd service file (Flowdalic)
