#!/usr/bin/env bash
# Copyright (c) 2016 The TheBurningSavage Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   theburningsavagecore.org and theburningsavage.org and compares them.
...
WORKINGDIR="/tmp/theburningsavage_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://theburningsavagecore.org"
HOST2="https://theburningsavage.org"
BASEDIR="/bin/"
VERSIONPREFIX="theburningsavage-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'theburningsavage-',
...
if [ $? -ne 0 ]; then
   echo "theburningsavage.org failed to provide signature file, but theburningsavagecore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "theburningsavage.org and theburningsavagecore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the TheBurningSavage Core binary release signing key installed?"
   fi
