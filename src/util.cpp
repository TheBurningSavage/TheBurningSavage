...
const char * const THEBURNINGSAVAGE_CONF_FILENAME = "theburningsavage.conf";
const char * const THEBURNINGSAVAGE_PID_FILENAME = "theburningsavaged.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "theburningsavaged -foo=bar
...
#else
    const char* pszModule = "theburningsavage";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TheBurningSavage
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TheBurningSavage
    // Mac: ~/Library/Application Support/TheBurningSavage
    // Unix: ~/.theburningsavage
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TheBurningSavage";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TheBurningSavage";
#else
    // Unix
    return pathRet / ".theburningsavage";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", THEBURNINGSAVAGE_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", THEBURNINGSAVAGE_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure TheBurningSavage Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("TheBurningSavage Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The TheBurningSavage Core developers";
    }
