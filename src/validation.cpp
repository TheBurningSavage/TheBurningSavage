...
#if defined(NDEBUG)
# error "TheBurningSavage cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "TheBurningSavage Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s TBS additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Subsidy is cut in half every 370,000 blocks which will occur approximately every 4 years.
    if (nHeight==1)nSubsidy = 2500000 * COIN;else nSubsidy >>= halvings;

    //total supply = 370000 * 25 * 2 + 2500000 = 2100000
    return nSubsidy;
}
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                if (!is_spent || pindex->nHeight != coin.nHeight || is_coinbase != coin.fCoinBase) {
...
void ThreadScriptCheck() {
    RenameThread("theburningsavage-scriptch");
