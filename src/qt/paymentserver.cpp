...
const int THEBURNINGSAVAGE_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString THEBURNINGSAVAGE_IPC_PREFIX("theburningsavage:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/theburningsavage-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/theburningsavage-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/theburningsavage-paymentrequest";
...
{
    QString name("TheBurningSavageQt");
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the theburningsavage: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(THEBURNINGSAVAGE_IPC_PREFIX, Qt::CaseInsensitive)) // theburningsavage: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseTheBurningSavageURI(arg, &r) && !r.address.isEmpty())
            {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(THEBURNINGSAVAGE_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(THEBURNINGSAVAGE_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click theburningsavage: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start theburningsavage: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling theburningsavage: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in theburningsavage: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("theburningsavage://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'theburningsavage://' is not a valid URI. Use 'theburningsavage:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(THEBURNINGSAVAGE_IPC_PREFIX, Qt::CaseInsensitive)) // theburningsavage: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseTheBurningSavageURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid TheBurningSavage address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom theburningsavage addresses are not supported
        ...
        // TheBurningSavage amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, optionsModel->node().getDustRelayFee())) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(TheBurningSavageUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
