 ...
 * between theburningsavaged, and theburningsavage-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("theburningsavage-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", THEBURNINGSAVAGE_CONF_FILENAME), false, OptionsCategory::OPTIONS);
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", THEBURNINGSAVAGE_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/theburningsavage/theburningsavage>";
    const std::string URL_WEBSITE = "<https://theburningsavagecore.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("theburningsavage-loadblk");
...
/** Sanity checks
 *  Ensure that TheBurningSavage is running in a usable environment with all
...
{
    // Make sure only a single TheBurningSavage process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", THEBURNINGSAVAGE_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if theburningsavage is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if theburningsavage is started while in a temporary directory.\n",
